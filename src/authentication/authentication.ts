...
userDataAxios.interceptors.request.use(req => {
    ...
    return { ...req, signal: controller.signal, headers: { authorization: `Bearer ${token}` } };
}, (error) => {
...
export const changeUserName = (username: string) =>
    userDataAxios.post<{ user: UserModel, token: string }>(`${appUrl}/user/change-username?username=${username}`)
        .then((res: AxiosResponse<{user: UserModel, token: string}>) => res.data)
        .catch(e => {
            throw new Error(e);
        });

export const checkUsername = (username: string) => userDataAxios.get(`${appUrl}/user/check-username`, { params: { username } });
